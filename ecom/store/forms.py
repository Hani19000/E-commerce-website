...
class SignUpForm(UserCreationForm):
	email = forms.EmailField(
		...
		required=True  # Changé à True pour forcer l'email
	)
	first_name = forms.CharField(label="First Name", max_length=100, widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'John', 'id':'first_name'}), required=True)
	last_name = forms.CharField(label="Last Name", max_length=100, widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'Doe', 'id':'last_name'}), required=True)
	...
	def clean_email(self):
		"""Valide que l'email est un compte Gmail"""
		...
		if not email.lower().endswith('@gmail.com'):
			raise ValidationError("Only Gmail addresses (@gmail.com) are allowed for registration.")
